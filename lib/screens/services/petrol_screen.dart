...
import '../../core/repositories/petrol_repository.dart';
...
/// The screen allowing residents to submit a Petrol (fuel delivery) request.
class PetrolScreen extends StatefulWidget {
  final PetrolRepository? repository;
...
class _PetrolScreenState extends State<PetrolScreen> {
  static const List<String> _standardQuantities = [
    '5 L',
    '10 L',
    '20 L',
    '30 L',
  ...
  String _selectedQuantity = '20 L';
  final _customQuantityController = TextEditingController();
  final _vehicleController = TextEditingController();
  final _notesController = TextEditingController();

  PetrolTiming _selectedTiming = PetrolTiming.asSoonAsPossible;
  ...
  PetrolRepository get _repo =>
      widget.repository ?? LocalPetrolRepository.instance;
  ...
  void _submit() {
    final isCustom = _selectedQuantity == 'CUSTOM';
    final customVal = _customQuantityController.text.trim();

    if (isCustom && customVal.isEmpty) {
      setState(() {
        _quantityError = AppStrings.errorPetrolQuantityRequired;
      });
      return;
    } else {
      setState(() {
    ...
    final effectiveQuantity = isCustom
        ? (customVal.toUpperCase().endsWith('L') ? customVal : '$customVal L')
        : _selectedQuantity;

    DateTime? scheduledDateTime;
    if (_selectedTiming == PetrolTiming.scheduled) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppHeader(
        ...
        subtitle: AppStrings.petrolHeaderSubtitle,
        showBackButton: true,
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                AppStrings.petrolQuestionSubtitle,
                style: TextStyle(
              ...
              // ── Fuel Quantity Grid ────────────────────────────────────
              _buildSectionLabel(AppStrings.labelFuelQuantity),
              const SizedBox(height: 8),
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: _standardQuantities.map((qty) {
              ...
              if (isCustom) ...[
                ...
                _buildSectionLabel(AppStrings.labelCustomLiters),
                const SizedBox(height: 8),
                AppTextField(
                  controller: _customQuantityController,
                  hintText: AppStrings.hintCustomPetrolQuantity,
              ...
              // ── Optional Vehicle ──────────────────────────────────────
              _buildSectionLabel(AppStrings.labelVehicleOptional),
              const SizedBox(height: 8),
              AppTextField(
                controller: _vehicleController,
                hintText: AppStrings.hintVehicle,
                minLines: 1,
                maxLines: 2,
              ...
              AppTextField(
                ...
                hintText: AppStrings.hintPetrolNotes,
                minLines: 2,
                maxLines: 4,
              ...
              // ── Submit Action ─────────────────────────────────────────
              AppButton(
                text: AppStrings.actionRequestPetrol,
                onPressed: _submit,
              ),
