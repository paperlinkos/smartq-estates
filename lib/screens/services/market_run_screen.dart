...
import '../../core/repositories/market_run_repository.dart';
...
/// The screen allowing residents to submit a Market Run shopping request.
class MarketRunScreen extends StatefulWidget {
  final MarketRunRepository? repository;

  const MarketRunScreen({
    super.key,
    this.repository,
  });

  @override
  State<MarketRunScreen> createState() => _MarketRunScreenState();
}

class _MarketRunScreenState extends State<MarketRunScreen> {
  final _itemsController = TextEditingController();
  final _notesController = TextEditingController();

  MarketRunTiming _selectedTiming = MarketRunTiming.asSoonAsPossible;
  ...
  MarketRunRepository get _repo =>
      widget.repository ?? LocalMarketRunRepository.instance;
  ...
  void _submit() {
    ...
    if (items.isEmpty) {
      setState(() {
        _itemsError = AppStrings.errorItemsRequired;
      });
      return;
    } else {
      setState(() {
    ...
    if (_selectedTiming == MarketRunTiming.scheduled) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppHeader(
        ...
        subtitle: AppStrings.marketRunHeaderSubtitle,
        showBackButton: true,
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                AppStrings.marketRunQuestionSubtitle,
                style: TextStyle(
              ...
              // ── Items Multiline Input ─────────────────────────────────
              _buildSectionLabel(AppStrings.labelItems),
              const SizedBox(height: 8),
              AppTextField(
                controller: _itemsController,
                hintText: AppStrings.hintMarketRunItems,
                errorText: _itemsError,
                minLines: 4,
                maxLines: 8,
                textCapitalization: TextCapitalization.sentences,
                onChanged: (val) {
                  if (_itemsError != null && val.trim().isNotEmpty) {
              ...
              AppTextField(
                ...
                hintText: AppStrings.hintMarketRunNotes,
                minLines: 2,
                maxLines: 4,
                textCapitalization: TextCapitalization.sentences,
              ),
              ...
              // ── Submit Action ─────────────────────────────────────────
              AppButton(
                text: AppStrings.actionRequestMarketRun,
                onPressed: _submit,
              ),
